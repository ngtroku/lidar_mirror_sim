...
if __name__ == "__main__":
    ...
    # visualize simulated points(world)
    world_simulated = o3d.geometry.PointCloud()
    vis.add_geometry(world_simulated)

    # visualize simulated points(local)
    local_simulated = o3d.geometry.PointCloud()
    vis.add_geometry(local_simulated)
    ...
    with AnyReader([bag_path], default_typestore=typestore) as reader:
        ...
        for connection, timestamp, rawdata in reader.messages(connections=connections):
            ...
            if connection.topic == point_topic:
                ...
                local_x, local_y, local_z = coord_trans.world_to_local(simulated_points, sensor_position, sensor_orientation)
                local_simulated_points = np.vstack((local_x, local_y, local_z)).T

                # --- Update Visualizer ---

                world_simulated.points = o3d.utility.Vector3dVector(simulated_points)
                world_simulated.paint_uniform_color([1.0, 0.0, 0.0]) # 赤色
                vis.update_geometry(world_simulated)

                local_simulated.points = o3d.utility.Vector3dVector(local_simulated_points)
                local_simulated.paint_uniform_color([0.0, 1.0, 0.0]) # 緑色
                vis.update_geometry(local_simulated)
