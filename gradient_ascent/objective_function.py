...
def simulator(param_x, param_y, param_yaw_center, param_swing_speed=7.0, param_swing_range=120):
    ...
    # load svms
    temp_path = Path(conditions['main']['temp_path'])
    temp_smvs = load_files.load_smvs(temp_path)
    ...
    occlusion_score_num, reflect_score_num = 0, 0
        ...
        for i, (connection, timestamp, rawdata) in enumerate(msg_iter):
            ...
            if connection.topic == lidar_topic_in:
                if cnt > max_frame_seq:
                    ...
                    #print(f"Frame {cnt}: # of occluded points: {P_occlusion.shape[0]}, occlusion score: {occlusion_score}")
                    ...
                    if is_mirror_visible_los:
                        ...
                        # load smvs
                        current_smvs = temp_smvs[load_num] # 現在のフレームのsmvs値
                        ...
                        P_virtual_raw, _ = reflection_sim(map_points_np, sensor_pos, sensor_quat, 
                        ...
                        if P_virtual_fov.shape[0] != 0:
                            #reflect_points_dist_mean, reflect_points_dist_std = test_func.point_stats(P_virtual_fov)
                            #reflect_score = (P_virtual_fov.shape[0] * reflect_points_dist_std)

                            reflect_score = P_virtual_fov.shape[0] # 鏡像反射により注入される点群数

                            #print(f"cnt:{cnt} & number of reflected points: {P_virtual_fov.shape[0]}")
                            #print(f"# of reflected points:{P_virtual_fov.shape[0]}, mean dist: {reflect_points_dist_mean:.2f}, std dist: {reflect_points_dist_std:.2f}, reflect score: {reflect_score:.2f}")
                        else:
                            reflect_score = 0

                        #print(f"Frame {cnt}: # of reflected points:{P_virtual_fov.shape[0]}, current SMVS: {current_smvs:.4f}")

                        weight_smvs = (reflect_score / lidar_points_world.shape[0]) # 入力点群に対する反射点群の割合で重みづけ
                        smvs_score = weight_smvs * current_smvs

                        reflect_score_num += (reflect_score - smvs_score) # SMVSの値が小さいほど脆弱な環境なので、smvsが大きいほどscoreを小さくする
                        #print(f"Frame {cnt}: # of reflected points:{P_virtual_fov.shape[0]}, reflect score: {reflect_score:.2f}, current total score: {occlusion_score_num + reflect_score_num:.2f}")
